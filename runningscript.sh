...
scp "$WORKSPACE/docker-compose.yml" $ENVFILE_LOCATION ec2-user@$machine:
...
if [ $machine == "testserver" ]; then
   ...
   ssh ec2-user@$machine "mkdir -p app; docker login;docker pull lironv/attendance:latest; docker-compose up -d --no-build;
       sleep 70;set -e; bash testfile.sh;sleep 5; docker-compose down;sleep 5; docker image rm lironv/attendance:latest; docker volume prune -f"
fi
